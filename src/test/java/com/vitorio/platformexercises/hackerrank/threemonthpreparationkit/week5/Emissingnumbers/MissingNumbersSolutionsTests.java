...
public class MissingNumbersSolutionsTests {

    private static final int INPUTS_AND_OUTPUTS = 2;
    private InputOutputFileReader fileReader;

    private final MissingNumbersSolution solution = new MissingNumbersSolution();
    private final MissingNumbersRemoveElementsFromListSolution removeElementsFromListSolution = new MissingNumbersRemoveElementsFromListSolution();

    @BeforeEach
    void setUp() {
        fileReader = new InputOutputFileReader(
            "/platformexercises/hackerrank/threemonthpreparationkit/week5/Emissingnumbers/"
        );
    }
    ...
    @ParameterizedTest
    @MethodSource("inputsOutputsSource")
    void testSolution_RemoveElementsFromList(String input, String output) throws IOException {
        // Given
        File inputContent = fileReader.fileFromFilePath(input);
        String[] outputContent = fileReader.readFileContent(output);
        StringBuilder expected = new StringBuilder();
        ...
        // When
        String actual = removeElementsFromListSolution.main(inputContent);
